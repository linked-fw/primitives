...
test('the ontology registers under @_linked/primitives, not @_linked/core', async () => {
  await import(
    new URL('../lib/esm/ontologies/primitives.register.js', import.meta.url)
  );
  const modules = globalThis._linked?._modules ?? {};

  assert.ok(
    modules['@_linked/primitives']?.radix,
    'the radix ontology is not registered under @_linked/primitives',
  );
  assert.ok(
    !modules['@_linked/core']?.radix,
    'the radix ontology leaked into @_linked/core',
  );
});
